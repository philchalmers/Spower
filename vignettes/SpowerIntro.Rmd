...
- A suitable $p$-value under the null hypothesis, $P(X|H_0)$, where the value of `sig.level` ($\alpha$) indicates whether a sample was ($P(D|H_0)<\alpha$) or was not ($P(D|H_0)\ge\alpha$) significant,
- The posterior probability of an alternative hypothesis, $P(H_1|D)$, which is deemed 'significant' if greater than the defined `sig.level` to indicate that a sample supported ($P(H_1|D)>\alpha$) or did not support ($P(H_1|D)\le\alpha$) the hypothesis of interest (requires setting `sig.direction = 'above'`), or
...
Were the power analysis in the context of a Bayesian analysis, the conditional probability of the alternative, $P(H_1|D)$, can be used instead. However, for this to work with *Spower* the argument `sig.direction = 'above'` should be used, where `sig.level` is used to flag significance above this set value. The purpose of using `sig.level` in this way is to evaluate whether the sample-based posterior probability favoring the hypothesis of interest is greater than this cut-off, so for example `sig.level = .95` would indicate "significant" support for the hypothesis whenever the sample-based posterior probability is greater than 95%. 
