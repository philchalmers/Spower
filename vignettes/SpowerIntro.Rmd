...
- A suitable $p$-value under the null hypothesis ($P(X|H_0))$), or a logical value also reflecting whether the null hypothesis was rejected (TRUE) or not (FALSE), or
- In the Bayesian setting, the posterior probability of the null hypothesis ($P(H_0|X)$), the complement of the alternative hypothesis of interest $P(\neg H_1|X) =1 - P(H_1|X)$, where $P(H_1|X)$ is the posterior probability supporting the alternative hypothesis), or a logical value indicating support for the alternative hypothesis

When a $p$-value/probability compliment value is return a specific $\alpha$ criteria will be used to construct a suitable logical in the package, where values less than this $\alpha$ are deemed 'significant' or 'supportive of the hypothesis $H_0$' in the null hypothesis and Bayesian testing paradigms, respectively. Note that more than one $p$-value/`logical` value may of course be returned by the functions, however for simplicity this vignette will focus on simple single-valued functions.
...
Were the power analysis in the context of a Bayesian analysis, the conditional probability of the alternative, $P(H_1|X)$, can be used instead. However, for this to work with *Spower* the argument `posterior.sig` should be used instead of the `sig.level`, the ladder of which is ignored in the Bayesian context. The purpose of `posterior.sig` is to evaluate whether the sample-based posterior probability favoring the hypothesis of interest is greater than or equal to this value, so for example `posterior.sig = .95` would indicate "significant" support for the hypothesis whenever the sample-based posterior probability is greater than or equal to 95%. 

Below is the Bayesian approach using the `BayesFactor` package, translating the output into a posterior probability and focusing on the alternative hypothesis that $H_1: \mu \ne 0$ (hence, the posterior probability returned is $P(\mu \ne 0|X)$). This also assumes that the competing hypotheses are equally likely (hence, prior odds are 1:1).
...
For the Bayesian $t$-test definition defined above, where "significance" in this context is flagged whenever the sample posterior is greater than .90 in support of $H_1$. Note that this is a more strict criteria than the null hypothesis criteria above, and therefore has lower power.

```{r eval=eval}
# power when the cut-off for a significantly supportive posterior is > 0.90
p_single.Bayes.t(n=100, mean=.5, mu=.3) |> 
	Spower(posterior.sig = .90) -> prospectiveBayes
prospectiveBayes
```

```{r echo=FALSE}
if(eval) store$prospectiveBayes <- prospectiveBayes
