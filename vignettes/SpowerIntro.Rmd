...
- A suitable $p$-value under the null hypothesis, $P(X|H_0)$, where the value of `sig.level` ($\alpha$) indicates whether a sample was ($P(X|H_0)<\alpha$) or was not ($P(X|H_0)\ge\alpha$) significant,
- The posterior probability of an alternative hypothesis, $P(H_1|X)$, which is deemed 'significant' if greater than the defined `posterior.sig` ($\gamma$) to indicate that a sample supported ($P(H_1|X)>\gamma$) or did not support ($P(H_1|X)\le\gamma$) the hypothesis of interest, or
- A logical value indicating support for the hypothesis of interest (e.g., `TRUE` if rejecting the null; `TRUE` if supporting the alternative; some more complicated combination involving multiple hypothesis or regions or practical significance, etc)

The package largely focuses on the first point as these are more common applications of power analysis, though nothing precludes the package from more complex power analysis tests and experiments. See the vignette *"Simulation experiments with logical vector and Bayesian power analyses"* for further examples involving CIs, ROPEs, equivalence tests, and Bayes Factors.
...
Were the power analysis in the context of a Bayesian analysis, the conditional probability of the alternative, $P(H_1|X)$, can be used instead. However, for this to work with *Spower* the argument `posterior.sig` should be used (thereby ignoring `sig.level`). The purpose of `posterior.sig` is to evaluate whether the sample-based posterior probability favoring the hypothesis of interest is greater than this value, so for example `posterior.sig = .95` would indicate "significant" support for the hypothesis whenever the sample-based posterior probability is greater than 95%. 

Below is the Bayesian approach using the `BayesFactor` package, translating the Bayes factor output into a posterior probability and focusing on the alternative hypothesis (hence, the posterior probability returned is $P(\mu \ne 0|X)$). The following also assumes that the competing hypotheses are equally likely when obtaining the posterior probability (hence, prior odds are 1:1).
