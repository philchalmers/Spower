...
Were the power analysis in the context of a Bayesian analysis, the conditional probability of the alternative can be used instead. However, for this to work with *Spower* the built-in structure must use the *compliment* of the posterior probability, $P(\neg H_1|X)=1 - P(H_1|X)$. This is so that the value can be used
along with threshold $\alpha$ (`alpha` in `Spower()`), however in this context it indicates the *strength* of the evidence. 

Below is the Bayesian approach using the `BayesFactor` package, assuming that the competing hypotheses are equally likely (hence, prior odds are 1:1).
...
p_single.Bayes.t <- function(n, mean, mu=0){
	...
	1 - posterior   # compliment, P(-H_1|X) = 1 - P(H_1| X)
}
```
