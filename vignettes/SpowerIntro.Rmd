...
The package defaults primarily focus on the first approach, as this is the most common approach for power analysis evaluation, however nothing precludes the `Spower` from more complex power analysis tests and experiments. See the vignette *"Bayesian power analyses, Type M and S errors, and ROPEs"* for further examples involving confidence intervals (CIs), regions of practical equivalences (ROPEs), equivalence tests, precision tests,  Bayes Factors, and Type S/M errors.
