...
However, Bayesian power analysis are also supported by the package, where for instance the posterior probability of the alternative hypothesis, $P(H_1|D)$, is the outcome of the simulation experiment. Below we demonstrate how to use *Spower* in this context. See the next section regarding similar approaches that rely on Bayes Factors instead.

Continuing with the simple one-sample $t$-test example in the introduction vignette, were the power analysis context be that of a Bayesian analysis the conditional probability of the alternative, $P(H_1|D)$, may be used instead. However, for this to work with *Spower* the argument `sig.direction = 'above'` should be used, where `sig.level` now indicts that "significance" only occurs when an observation is above this cutoff. For example, if the simulation experiment returns a posterior probability $H_1$, $P(H_1|D)$, then `sig.level = .90` would indicate "significant" support for the hypothesis whenever the sample-based posterior probability is greater than `0.90`. 

Below is the Bayesian approach using the `BayesFactor` package, translating the Bayes factor output into a suitable posterior probability and focusing on the alternative hypothesis (hence, the posterior probability returned corresponds to $P(\mu \ne 0|D)$). The following also assumes that the competing hypotheses are equally likely when obtaining the posterior probability (hence, prior odds are 1:1).
...
For the Bayesian $t$-test definition defined above, where "significance" in this context is flagged whenever the sample posterior is greater than .90 in support of $H_1$, which in this example corresponds to $H_1:\,\mu\ne \mu_0$, where $\mu=.5$ and $\mu_0=0$. Note that this is a more strict criteria than the null hypothesis criteria presented in the introduction vignette, and therefore has notably lower power.

```{r eval=eval}
# power when the cut-off for a significantly supportive posterior is > 0.90
...
With this approach, the five describe power analysis criteria available through `Spower()` are now entirely possible, and therefore solving other components (such as the sample size `n`) are easy to setup.
