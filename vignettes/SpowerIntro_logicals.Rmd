---
title: "Logical vectors and Bayesian power analyses"
...
output: 
  bookdown::html_document2:
    ...
    toc: true
vignette: >
  %\VignetteIndexEntry{Logical vectors and Bayesian power analyses}
...
The canonical way that *Spower* has been written by focusing primarily on $p$-values involving the null hypothesis to be tested ($P(X|H_0)$), or by focusing on posterior probability values in Bayesian contexts to evaluate the support for the alternative hypothesis ($P(H_1|X)$). The reason for setting the package up this way is so that the parameter $\alpha$ parameter (`sig.level`), or the posterior probability (`posterior.sig`), can be used internally as a "line-in-the-sand" threshold to flag whether a hypothesis was rejected (under the null hypothesis) or supported (under posterior probability of the alternative hypothesis setup through Bayesian analyses). However, it is often advantageous to directly return `logical` values direction in the simulation experiment rather than letting `Spower()` perform these threshold transformations as these can include more intricate experimental result requirements. 
...
```{r eval=eval}
p_single.t(n=100, mean=.3) |> Spower() -> pPower
pPower
```

```{r echo=FALSE}
if(eval) store$pPower <- pPower
pPower <- store$pPower
print(pPower)
```

```{r eval=eval}
l_single.t(n=100, mean=.3) |> Spower() -> lPower
lPower
```

```{r echo=FALSE}
if(eval) store$lPower <- lPower
lPower <- store$lPower
print(lPower)
```
...
Similarly, if one were using a Bayesian analysis criteria, the Bayes factor (BF) ratio could be used in the logical return context. For example, returning whether the observed $BF>3$ in a given random sample indicates moderate or greater evidence for the hypothesis of interest compared to some competing hypothesis (often the null, though not necessarily). The downside is that BF require the computation of the marginal likelihood terms, typically via bridge sampling (e.g., via the `bridgesampling` package), in addition to fitting the model using Markov chain Monte Carlo methods (e.g., `brms`, `rstan`, `rstanarm`). Though not necessarily typically a limitation, it is often more natural to focus directly on the sample from posterior distribution for power analysis applications rather than on the marginal Bayes factors. Nevertheless, such applications are possible with `Spower` if there is interest in doing so.

As a simple example, the following one-sample $t$-test defined in the introduction vignette could be redefined from the original form
```{r eval=eval}
p_single.Bayes.t <- function(n, mean, mu=0){
	g <- rnorm(n, mean=mean)
...
```
to retain the Bayes factor result for comparison. Suppose now that one were interested in estimating the power to observe $BF>3$ across a set of independently drawn experiments. This can be achieved using the following:
```{r eval=eval}
...
Evaluating this simulation with $N=100$, $\mu=.5$, and $\mu_0=.3$ gives the following power estimate when evaluating each sample against a Bayes factor cut-off of 3. 

```{r eval=eval}
l_single.Bayes.t_BF(n=100, mean=.5, mu=.3) |> Spower() -> prospectiveBF3
prospectiveBF3
```

```{r echo=FALSE}
if(eval) store$prospectiveBF3 <- prospectiveBF3
prospectiveBF3 <- store$prospectiveBF3
print(prospectiveBF3)
```


# Equivalence testing and regions of practical equivalence (ROPEs)
...
l_equiv.t(100, delta=1, equiv=c(-2, 2))
l_equiv.t(500, delta=1, equiv=c(-2, 2), sds=c(2.5, 2.5))
```


```{r eval=eval}
l_equiv.t(100, delta=1, equiv=c(-2.5, 2.5), 
		  sds=c(2.5, 2.5)) |> Spower() -> equivT
equivT
```

```{r echo=FALSE}
if(eval) store$equivT <- equivT
equivT <- store$equivT
print(equivT)
```
...
l_equiv.t_CI(100, delta=1, equiv=c(-2.5, 2.5), 
		  sds=c(2.5, 2.5)) |> Spower() -> equivTL
equivTL
```

```{r echo=FALSE}
if(eval) store$equivTL <- equivTL
equivTL <- store$equivTL
print(equivTL)
```


## Region of Practical Equivalence (HDI + ROPE)
...
p_rope.lm(n=20, beta0=2, beta1=1, range=c(.8, 1.2))
p_rope.lm(n=100, beta0=2, beta1=1, sigma=1/2, range=c(.8, 1.2))
```
In the above example, the proportion of the sampled posterior distribution that falls within the ROPE is returned, which works well with the `posterior.sig` argument in `Spower()` to define a suitable accept/reject cut-off. Specifically, if `posterior.sig = .95` then the ROPE will only be accepted when the percentage of the posterior that falls within the defined ROPE is greater than 95%. This can of course be performed manullay instead, returning a `TRUE` when satisfied and `FALSE` otherwise.
...
p_rope.lm(n=50, beta0=2, beta1=1, sigma=1/2, range=c(.8, 1.2)) |> 
	Spower(posterior.sig=.95, parallel=TRUE) -> ROPE.lm
ROPE.lm
```

```{r echo=FALSE}
if(eval) store$ROPE.lm <- ROPE.lm
...
p_rope.lm(n=NA, beta0=2, beta1=1, sigma=1/2, range=c(.8, 1.2)) |> 
	Spower(power=.80, posterior.sig=.95, interval=c(50, 200), parallel=TRUE) -> ROPE.lm_n
ROPE.lm_n
```


```{r echo=FALSE}
if(eval) store$ROPE.lm_n <- ROPE.lm_n
