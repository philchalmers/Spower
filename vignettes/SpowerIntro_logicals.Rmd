...
In the publication *Gelman, A., & Carlin, J. (2014). Beyond Power
...
confidence that our sample size is large enough if we obtain reject the
null when the $M$ ratio is less than, say, 2, then `TRUE/FALSE` values
can be used in the configuration process. This is show in the second
demonstration below.

## Type S error demonstration

To demonstrate how to estimate Type S power, the following simulation
performs a two-sample $t$-test with a small effect size (Cohen's $d=.2$)
with a small sample size. The output returned is not intended to be
power in this case, but rather an estimate of the number of Type S
errors.
...
Here, the `power` element provides the estimate of the correct sign
decision given that "significance" has occurred, while $1-power$
provides the estimate of the associated Type S error. As can be seen in
this case, given that significance has been observed for the given
...
simply due to unreasonably high sampling variance. For the purpose of
this power demonstration, suppose "reasonably close" were defined such
that the ratio of the absolute mean difference in a two sample test
test, which was flagged as significant, is less than twice the value of
the true mean difference (hence, `M.ratio = 2`). For interest sake, it
is also possible to store the observed $M$ values if a `list` or
`data.frame` is returned from the experiment, however for this to behave
correctly the information relevant to the `power` computations
...
cutoff of 2 (hence, was less than twice the true effect size). The
...
With respect to the distribution of the observed $M$ values, these can
be extracted and further inspected by using `SimResults()`, which
demonstrate the following properties. Notice that the rejection
magnitudes are not uniformly distributed, which is to be expected given
the nature of the conditional simulation experiement.
...
issues, as seen below by doubling the sample size.
...
so approximately 26% of the significant results would have been due to
...
is likely more interpretable for data analysts to specify.
