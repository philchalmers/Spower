...
```{r eval=eval}
pp_single.Bayes.t <- function(n, mean, mu){
	g <- rnorm(n, mean=mean)
...
In the publication *Gelman, A., & Carlin, J. (2014). Beyond Power Calculations: Assessing Type S (Sign) and Type M (Magnitude) Errors. Perspectives on Psychological Science, 9(6), 641-651. https://doi.org/10.1177/1745691614551642*, the authors present power analysis ideas related to *Type S* and *Type M* errors, which add augmentations to the usual rejection of $H_0$ or support for $H_1$. Specifically:

- **Type S errors** occur when a null hypothesis is confidently rejected in light of the alternative being true, however the rejection is in the wrong direction of the underlying effect. Hence, conditional on a "statistical significance" decision, what is the probability that the correct sign was inferred?  Ideally Type S errors should be close to 0
	- This is particularly problematic in under powered research studies, where the sampling variability could result in detection of an effect that happens to be opposite to the underlying cause. 
- **Type M errors**, on the other hand, occur when the magnitude of a detected effect is much larger than the expected effect given that the "statistical significance" has occurred. Gelman and Carlin (2014) have presented this as a ratio idea ($E(\frac{|estimate|}{|parameter|})$), where large ratios indicate that the estimate had to be that many times higher than the expected value of the parameter (e.g., a Type M ratio of 8 indicates that of those effect sizes flagged as significant the effect estimate had to be on average 8 times larger than the expected parameter).
	- With the ratio gets at the idea nicely, nothing really precludes us from drawing further lines in the sand to better map this into a power framework rather than a ratio. For instance, if we would only be confidence that our sample size is large enough if we obtain  
...
```{r eval=eval}
l_single.t_correct.sign <- function(n, mean, alpha = .05, ...){
	while(TRUE){
		g1 <- rnorm(n)
...
```{r eval=eval}
l_single.t_correct.sign(n=15, mean=.2) |> Spower()
```
```{r echo=FALSE}
...
```{r eval=eval}
typeS <- .01
l_single.t_correct.sign(n=NA, mean=.2) |> 
