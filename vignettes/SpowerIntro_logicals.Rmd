...
letting `Spower()` perform these threshold transformations internally (using `sig.level`) as these can
...
(e.g., including `conf.level` or `sig.level` in the simulation definition directly).
...
If one were to take this approach, the defined function should instead return a
`logical` value based on the relation of the parameter estimate to the CI, 
where the CI is used to evaluate the
plausibility of $\mu = \mu_0$. Specifically, the CI is used in this context to evaluate whether $\mu_0$ falls *outside*
the advertised interval. Alternatively, if one were in a Bayesian analysis
...
`is.outside_CI()` function to evaluate whether the $\mu$ parameter 
falls within the estimated `CI` returned from `t.test()`.
...
Note that even in this context writing a user defined function is not necessary as the internally defined function `p_t.test()` can be used to obtain the same information by returning the model itself and subsequently extracting the `$conf.int` element. The benefit of this, as shown below, is that users do not need to reinvent the data generation and analysis portions of the experiment if this has already been defined in the package.

```{r}
l_single.t <- function(n, mean, mu=0, conf.level=.95){
	# return analysis model for further extraction
...
rejected (in the $p$-value context) *and* the CI is
sufficiently narrow.
...
significant result this joint criteria estimate requires a notably higher sample size. Note that, in the special case, if `CI.width=Inf` then all CI widths will be accepted, in which case the same results could have been evaluated using `p_single.t()` defined earlier.
...
and `FALSE` if less than or equal to 3. Evaluating this simulation with $N=100$,
...
The canonical way that *Spower* has been written is by focusing primarily
...
hypothesis, $P(H_1|D)$, may be the outcome of the simulation experiment. 
...
observation is *above* the `sig.level` cutoff.
...
As a running example, suppose that in an independent samples $t$-test situation two groups might be considered "equivalent" if the true mean difference in the population is
...
groups are to be deemed statistically equivalent given these boundary locations then the null hypotheses to test using the two
...
were determined to be practically equivalent a priori. What then would be the power to jointly rejecte the above null hypothesis and therefore conclude the groups are practically equivalence ($H_1$)?
...
Again, the same type of logic can be evaluated using `CI`s alone, where in this case `TRUE` is returned if the estimated `CI` falls within the defined equivalence interval.
