...
with a small sample size. Note the use of a `while()` loop to generate and analyse the experiment until a significant $p$-value is observed, at which point the `while()` loop is terminated and the subsequent analyses relevant to the compliment of Type S error (the correct sign decision) is returned as a `logical`.
...
l_two.t_correct.sign <- function(n, mean, mu = 0, alpha = .05, ...){
	...
	mean_diff <- out$estimate[2] - out$estimate[1]
	mean_diff > mu
}
```
...
As before, the first step is to define the experiemtn using the conditional
