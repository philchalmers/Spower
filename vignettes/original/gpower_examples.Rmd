...
2007) for the *Gaussian(0,1)* distribution with varying sample sizes and
...
The following partially recreates the simulation results in Figure 29
for the *Laplace(0,1)* distribution with varying sample sizes and effect
sizes. The target was to obtain "approximate power of $1-\beta = .80$",
though how these sample sizes were decided upon was not specified.
`Spower()`'s stochastic root-solving approach would likely get closer to
more optimal $N$ estimates were these the target of the analyses.
...
```{r}
library(extraDistr)
parent1 <- function(n, d) extraDistr::rlaplace(n)/sqrt(2) + d
...
parent1 <- function(n, d) extraDistr::rlaplace(n)/sqrt(2) + d
parent2 <- function(n, d) extraDistr::rlaplace(n)/sqrt(2)
...
with `Spower`, where a power of .847 is reported.
...
Again, the simulation approach and G\*power 3.1 differ in their ouputs,
where in G\*power 3.1 the power is reported to be 0.853.
