...
## Two-sample test with Laplace distributions (Example 23.3)

```{r}
library(VGAM)

# find VAR of distribution for standardizing
fn <- function(x) x^2 * dlaplace(x)
SD <- sqrt(integrate(fn, -Inf, Inf)$value)
SD

parent1 <- function(n, d) VGAM::rlaplace(n)/SD + d
parent2 <- function(n, d) VGAM::rlaplace(n)/SD
Spower(p_wilcox.test, n=10, d=1.13842, type='paired',
	   parent1=parent1, parent2=parent2, verbose=FALSE)
```

G\*power estimates power to be 0.853.
